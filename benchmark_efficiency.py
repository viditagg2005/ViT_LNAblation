import torch
import time
import numpy as np
import argparse
from torch.profiler import profile, record_function, ProfilerActivity
import timm

# NOTE: Import your model factory here if it's not in timm
# from models import create_model 

def measure_throughput(model, input_tensor, batch_size, mode='inference', warmup=10, runs=50):
    model.eval() if mode == 'inference' else model.train()
    
    # Warmup
    if torch.cuda.is_available(): torch.cuda.synchronize()
    for _ in range(warmup):
        out = model(input_tensor)
        if mode == 'train':
            loss = out.sum()
            loss.backward()
            model.zero_grad()
    
    # Timing
    if torch.cuda.is_available(): torch.cuda.synchronize()
    start_time = time.time()
    
    for _ in range(runs):
        out = model(input_tensor)
        if mode == 'train':
            loss = out.sum()
            loss.backward()
            model.zero_grad()
            
    if torch.cuda.is_available(): torch.cuda.synchronize()
    end_time = time.time()
    
    total_time = end_time - start_time
    avg_time_per_run = total_time / runs
    throughput = batch_size / avg_time_per_run
    
    return throughput, avg_time_per_run * 1000 # img/sec, ms

def measure_memory(model, input_tensor):
    torch.cuda.reset_peak_memory_stats()
    torch.cuda.empty_cache()
    model.train()
    
    # Run one step
    out = model(input_tensor)
    loss = out.sum()
    loss.backward()
    
    mem = torch.cuda.max_memory_allocated() / (1024 ** 2) # MB
    return mem

def profile_kernels(model, input_tensor):
    print("\n[INFO] Profiling Kernels... (Exporting chrome trace)")
    model.eval()
    with profile(activities=[ProfilerActivity.CUDA], record_shapes=True) as prof:
        with record_function("model_inference"):
            model(input_tensor)
            
    print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=15))
    prof.export_chrome_trace("efficiency_trace.json")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', type=str, default='vit_base_patch16_224')
    parser.add_argument('--dynamic_tanh', action='store_true', help='Use DyT')
    parser.add_argument('--batch_sizes', type=int, nargs='+', default=[1, 64])
    parser.add_argument('--image_size', type=int, default=224)
    args = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {device} | Model: {args.model} | DyT: {args.dynamic_tanh}")

    # Create Model - Adjust this line to match how your repo constructs models!
    # For standard DyT repo, you likely need to import specific model definitions.
    # Here we use timm as a placeholder. 
    try:
        # Trying standard timm creation
        model = timm.create_model(args.model, pretrained=False)
        # If you have a way to inject dynamic_tanh, do it here. 
        # e.g. model = create_dyntanh_model(...)
    except:
        print("Error creating model. Ensure you are importing your repo's model definition correctly.")
        exit()
        
    model.to(device)
    
    IMG_SHAPE = (3, args.image_size, args.image_size)

    print("-" * 60)
    print(f"{'Batch':<10} {'Mode':<10} {'Throughput (img/s)':<20} {'Latency (ms)':<15} {'Mem (MB)':<10}")
    print("-" * 60)

    for b in args.batch_sizes:
        input_t = torch.randn(b, *IMG_SHAPE).to(device)
        
        # Inference
        inf_thr, inf_lat = measure_throughput(model, input_t, b, mode='inference')
        print(f"{b:<10} {'Infer':<10} {inf_thr:<20.2f} {inf_lat:<15.2f} {'-':<10}")
        
        # Train
        try:
            train_thr, train_lat = measure_throughput(model, input_t, b, mode='train')
            mem = measure_memory(model, input_t)
            print(f"{b:<10} {'Train':<10} {train_thr:<20.2f} {train_lat:<15.2f} {mem:<10.2f}")
        except RuntimeError as e:
            print(f"{b:<10} {'Train':<10} OOM or Error: {e}")

    # Run Profiler on Batch 32
    if device == 'cuda':
        dummy = torch.randn(32, *IMG_SHAPE).to(device)
        profile_kernels(model, dummy)